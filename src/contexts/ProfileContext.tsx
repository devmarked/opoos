...
export function ProfileProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        ...
        if (createError) {
          ...
          sessionCache.set(cacheKey, newProfile, 10 * 60 * 1000) // 10 minutes
        }
      } else if (error) {
        console.error('Error fetching profile:', error)
      } else {
        setProfile(data)
        sessionCache.set(cacheKey, data, 10 * 60 * 1000) // 10 minutes
      }
    } catch (error) {
  ...
  const updateProfile = async (data: ProfileUpdateData) => {
    ...
    if (!error) {
      ...
      // Update cache
      const cacheKey = CACHE_KEYS.PROFILE(user.id)
      sessionCache.set(cacheKey, updatedProfile, 10 * 60 * 1000)
    }
